...
camera_params = [1388.0, 1398.0, 507.0, 364.0]
K = np.array([[1388.0, 0.0, 507.0], [0.0, 1398.0, 364.0], [0.0, 0.0, 1.0]])
...
class ARMaze:
    def __init__(self, rows=10, cols=10):
        ...
        self.__grids = list(
            product(range(row_lim - rows, row_lim + 1), range(col_lim - cols, col_lim + 1), (0,))
        )
        self.__2d_pts = None
    ...
    def __xy_to_world(self, x, y):
        return x - self.__rows + self.__rows // 2 + 0.5, y - self.__cols + self.__cols // 2 + 0.5, 0
    ...
    def __reshaped_pts(self):
        return [
            [self.__2d_pts[i * (self.__cols + 1) + j] for j in range(self.__cols + 1)]
    ...
    def show_window(self):
        ...
        with RPiCamera("tcp://192.168.1.162:65433") as camera:
            while True:
                if self.__thread:
                ...
                tags = at_detector.detect(
                    cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), True, camera_params, 0.5
                )
                self.__2d_pts = None
                if len(tags) != 0:
                    tag = tags[0]
                    ...
                    if self.__start is not None:
                        cv2.circle(
                            img,
                            convert_3d_to_2d(p, [self.__xy_to_world(*self.__start)])[0],
                            5,
                            (255, 0, 0),
                            5,
                        )
                    if self.__goals:
                        for goal in convert_3d_to_2d(
                            p, list(map(lambda t: self.__xy_to_world(*t), self.__goals))
                        ):
                            cv2.circle(img, goal, 5, (0, 0, 255), 5)
                    if self.__path is not None:
                        order = convert_3d_to_2d(
                            p, list(map(lambda t: self.__xy_to_world(*t), self.__path[0]))
                        )
                        path = convert_3d_to_2d(
                            p, list(map(lambda t: self.__xy_to_world(*t), self.__path[1]))
                        )
                ...
                if key == ord("q"):
                    ...
                    if not self.__path:
                        ...
                        self.__thread = threading.Thread(
                            target=droid_roll, args=(self.__path[1],)
                        ).start()
